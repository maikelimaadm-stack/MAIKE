...
export default function RateioGruposSection({ rateios, onChange, grupos, valorTotal }) {
  const adicionarRateio = () => {
    onChange([...rateios, { grupo_financeiro_id: '', grupo_financeiro_nome: '', valor: 0, percentual: 0 }]);
  };
  ...
  return (
    <div className="space-y-2 border border-slate-200 rounded-lg p-2.5 bg-white">
      <div className="flex justify-between items-center">
        <h3 className="text-xs font-bold text-slate-700 uppercase">Rateio por Grupo Financeiro</h3>
      ...
      {rateios.map((rateio, index) => (
        <div key={index} className="grid grid-cols-12 gap-1.5 items-end bg-slate-50 rounded p-1.5">
          <div className="col-span-5 space-y-1">
            ...
            <Select value={rateio.grupo_financeiro_id} onValueChange={(v) => atualizarRateio(index, 'grupo_financeiro_id', v)}>
