...
import { CheckCircle, Save, X, AlertCircle, Edit, Trash2, Paperclip, FileText, Calendar } from "lucide-react";
...
export default function BaixaFinanceira({ lancamento, onClose, onSuccess }) {
  ...
  const baixaMutation = useMutation({
    mutationFn: async (data) => {
      ...
      // Se foi parcial E tem próxima previsão, atualizar data vencimento
  ...
  const handleSubmit = (e) => {
    ...
    // Verificar se é parcial
    const saldoAposBaixa = saldoDisponivel - valorBaixa;
    const ehParcial = saldoAposBaixa > 0.01;

    if (ehParcial && !formData.proxima_previsao) {
      toast.error('Para baixa parcial, informe a próxima previsão de pagamento!');
      return;
    }
  ...
  // Calcular se será parcial
  const saldoDisponivel = (lancamento.valor_total || 0) - (lancamento.valor_pago || 0);
  const valorBaixaAtual = parseMoeda(formData.valor_baixa);
  const saldoAposBaixa = saldoDisponivel - valorBaixaAtual;
  const ehParcial = saldoAposBaixa > 0.01;
                ...
                <AlertDescription>
                  <div className="space-y-2">
                    <p className="text-xs font-semibold text-orange-800">
                      ⚠️ Baixa Parcial - Saldo restante: {formatarMoeda(saldoAposBaixa)}
                    </p>
                    <div className="space-y-1">
                      <Label className="text-xs font-semibold text-orange-900">Próxima Previsão de Pagamento *</Label>
                      <Input 
                        type="date" 
