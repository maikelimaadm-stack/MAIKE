    ...
    Object.entries(lotesPorArea).forEach(([areaId, lotesNaArea]) => {
      ...
      if (identificadores.length > 0) {
        ...
        if (indicatorOverlay._state !== stateStr) {
          indicatorOverlay._div.innerHTML = identificadores.map((i) => `<div title="${i.nome || i.sigla || ''}" style="width:5px;height:5px;border-radius:9999px;background-color:${i.cor};border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:bold;color:#fff;line-height:1;overflow:hidden;">${i.sigla ? String(i.sigla).substring(0,3) : ''}</div>`).join('');
