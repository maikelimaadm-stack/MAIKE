      ...
      <div className="rounded-xl border bg-card shadow-sm">
        ...
        <div className="p-3 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant={filtroStatus === 'ativas' ? 'default' : 'outline'} size="sm" onClick={() => setFiltroStatus('ativas')} className="h-7 text-xs">Ativas</Button>
            <Button variant={filtroStatus === 'concluidas' ? 'default' : 'outline'} size="sm" onClick={() => setFiltroStatus('concluidas')} className="h-7 text-xs">Concluídas</Button>
            <Button variant={filtroStatus === 'todas' ? 'default' : 'outline'} size="sm" onClick={() => setFiltroStatus('todas')} className="h-7 text-xs">Todas</Button>
          </div>
                    ...
                    return (
                      <TableRow key={tarefa.id} className="data-[state=selected]:bg-muted transition-colors border-b hover:bg-gray-100" onDoubleClick={() => abrirDetalhe(tarefa)} onTouchEnd={(event) => handleCardTouch(tarefa, event)}>
                        ...
                        <TableCell className="p-2 text-gray-700 text-xs align-middle px-2 h-7 border border-gray-300">
                          <Badge variant="outline" className={`text-[10px] border-transparent shadow-none ${prioridadeClassName}`}>{prioridade || '-'}</Badge>
                        </TableCell>
                        <TableCell className="p-2 text-gray-700 text-xs align-middle px-2 h-7 border border-gray-300">
                          <Badge variant="outline" className={`text-[10px] border-transparent shadow-none ${STATUS_CORES[tarefa.status] || STATUS_CORES.Pendente}`}>{tarefa.status || '-'}</Badge>
                        </TableCell>
