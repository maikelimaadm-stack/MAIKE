        ...
        <Card className="rounded-xl border bg-card text-card-foreground shadow">
          <CardContent className="p-3">
            <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
              <div className="md:col-span-2 space-y-1"><Label className="text-xs uppercase">Buscar</Label><Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="BUSCAR LINHAS..." className="h-7 text-xs uppercase" /></div>
              <div className="space-y-1"><Label className="text-xs uppercase">Tipo</Label><Select value={filtroTipo} onValueChange={setFiltroTipo}><SelectTrigger className="h-7 text-xs uppercase"><SelectValue placeholder="TODOS" /></SelectTrigger><SelectContent><SelectItem value={VALOR_TODOS} className="text-xs uppercase">TODOS</SelectItem>{tipos.map((item) => <SelectItem key={item} value={item} className="text-xs uppercase">{item}</SelectItem>)}</SelectContent></Select></div>
            </div>
