...
import { OPERACOES_CALCULO, montarFormulaVisual } from "./camposConfigOptions";
...
export default function VisualCalculationBuilder({ value = [], fields = [], onChange }) {
  ...
  return (
    <div className="p-2 border rounded-lg bg-white space-y-2">
          ...
          <div key={index} className="grid grid-cols-[90px_1fr_34px] gap-2 items-center">
            ...
            <Select value={item.field || "none"} onValueChange={(field) => updateItem(index, { field: field === "none" ? "" : field })}>
              ...
              <SelectContent>
                ...
                {fields.map((field) => <SelectItem key={field.value} value={field.value} className="text-xs">{field.label}</SelectItem>)}
              </SelectContent>
            </Select>
      ...
      <div className="border rounded bg-slate-50 px-2 py-1 text-xs text-slate-700">
        <span className="font-semibold">Prévia:</span> {montarFormulaVisual(items) || "selecione os campos"}
      </div>
    </div>
  );
}