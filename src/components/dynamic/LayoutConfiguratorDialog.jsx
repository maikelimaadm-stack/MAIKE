  ...
  const renderPanelField = (field, index) => {
    ...
    return (
      <button
        ...
        onDragEnd={() => setDraggedFieldId(null)}
        className={` ${draggedFieldId === field.id ? "opacity-50 scale-95 relative h-8 min-w-[210px] px-2 rounded-sm text-left border items-center justify-between overflow-hidden bg-gray-50 border-slate-200 ring-2 ring-green-500 transition-all text-[hsl(var(--foreground))] fle" : ""} ${selected ? "" : ""} ${hidden ? "bg-slate-100 text-slate-400 border-slate-300" : required ? "" : "bg-green-500 border-green-500 hover:bg-green-600"}`}>
