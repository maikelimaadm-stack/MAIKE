...
function formatObsKgPtBR(obs) {
  if (!obs) return "";
  // Replace patterns like "100.000 kg" or "50.500 kg" with pt-BR format "100,000 kg"
    ...
    queryFn: async () => {
      ...
      return all
        .filter((item) => {
          ...
          // For transfers: only show the side relevant to this deposit
          // If deposit is ORIGIN → show the saída (transferencia_enviada)
          // If deposit is DESTINATION → show the entrada (transferencia_recebida)
          ...
          // For suplementação (saída from deposit to cocho), deposit is origin
