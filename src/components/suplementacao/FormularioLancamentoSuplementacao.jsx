      ...
      <Card>
        ...
        <CardContent className="p-4 max-h-[calc(100vh-200px)] overflow-y-auto">
          <div className="space-y-4">
            ...
            {formData.quantidade_total_kg && totalCabecas > 0 && lotes.length > 0 && <div className="bg-slate-50 border border-slate-300 rounded-lg p-3 space-y-3"><div className="text-xs font-semibold text-slate-900">Distribuição ponderada do novo período</div><div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[10px]"><div className="rounded-md border border-slate-200 bg-white p-2"><div className="text-slate-500">Fornecido agora</div><div className="font-bold text-slate-900">{formatDecimal(quantidadeTotal)} kg</div></div><div className="rounded-md border border-slate-200 bg-white p-2"><div className="text-slate-500">Sobra inicial do período</div><div className="font-bold text-slate-900">{formatDecimal(sobraInformada)} kg</div></div><div className="rounded-md border border-slate-200 bg-white p-2"><div className="text-slate-500">Oferta total do período</div><div className="font-bold text-slate-900">{formatDecimal(quantidadeTotal + sobraInformada)} kg</div></div><div className="rounded-md border border-slate-200 bg-white p-2"><div className="text-slate-500">Peso total consumo</div><div className="font-bold text-slate-900">{formatDecimal(pesoTotalConsumo)}</div></div></div><div className="space-y-2">{lotes.map((lote) => { const fator = fatores.find((item) => normalizeText(item.categoria) === normalizeText(lote.categoria))?.fator || 0; const pesoConsumoLote = (lote.quantidade_cabecas || 0) * fator; const percentualConsumo = pesoTotalConsumo > 0 ? (pesoConsumoLote / pesoTotalConsumo) * 100 : 0; const consumoEstimadoLoteDia = safeDivide(consumoEstimadoPeriodoKg * percentualConsumo / 100, diasEstimadosNovoPeriodo); return <div key={lote.id} className="bg-white border border-slate-200 rounded-md p-2"><div className="flex items-center justify-between mb-1"><div className="font-semibold text-xs text-slate-900">{lote.nome}</div><Badge variant="outline" className="text-[10px]">{lote.categoria}</Badge></div><div className="grid grid-cols-4 gap-2 text-[10px]"><div><div className="text-slate-500">Cabeças</div><div className="font-bold text-slate-900">{formatDecimal(lote.quantidade_cabecas || 0, 0, true)}</div></div><div><div className="text-slate-500">Fator</div><div className="font-bold text-slate-900">{formatDecimal(fator, 2)}</div></div><div><div className="text-slate-500">% Consumo</div><div className="font-bold text-emerald-700">{formatDecimal(percentualConsumo, 1)}%</div></div><div><div className="text-slate-500">Kg/dia estimado</div><div className="font-bold text-slate-900">{formatDecimal(consumoEstimadoLoteDia, 3)}</div></div></div></div>; })}</div></div>}
