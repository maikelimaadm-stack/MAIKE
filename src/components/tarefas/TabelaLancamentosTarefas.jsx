    ...
    <div className="space-y-1">
      {showHeaderActions && (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 bg-white rounded px-1 py-1 shadow-sm border-b border-slate-200">
          <div>
            <h1 className="text-sm font-bold text-slate-900">{headerTitle}</h1>
            {headerDescription ? <p className="text-xs text-slate-600 uppercase">{headerDescription}</p> : null}
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="icon" onClick={() => setShowConfigColunas(true)} className="h-7 w-7">
              <MoreVertical className="w-4 h-4" />
            </Button>
            <Button size="sm" onClick={onAdd} className="bg-lime-900 text-primary-foreground px-3 text-xs font-medium rounded-md inline-flex items-center justify-center gap-2 whitespace-nowrap transition-colors shadow h-7 hover:bg-emerald-600">
              Adicionar
            </Button>
          </div>
        </div>
              ...
              <TableHeader className="sticky top-0 z-30 bg-white shadow-sm">
                <TableRow className="bg-white border-b">
                  {colunasOrdenadas.map((coluna) => {
                    if (coluna.id === "selecao") return <TableHead key="selecao" className="p-0 bg-white text-muted-foreground font-medium text-center sticky left-0 z-40 w-10 min-w-[40px] max-w-[40px] align-middle px-0 border border-gray-300"><Checkbox checked={selectedItems.length === tarefasFiltradas.length && tarefasFiltradas.length > 0} onCheckedChange={toggleSelectAll} className="peer shrink-0 shadow disabled:opacity-50 h-4 w-4 rounded-full border-2 border-gray-400 shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 disabled:cursor-not-allowed disabled:opacity-70 data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground" /></TableHead>;
                    if (coluna.id === "acoes") return <TableHead key="acoes" className="h-10 p-0 bg-white text-muted-foreground font-medium text-center sticky left-[40px] z-40 w-10 min-w-[40px] max-w-[40px] align-middle px-0 border border-gray-300"></TableHead>;
                    return <TableHead key={coluna.id} className="h-7 align-middle text-gray-900 px-1 text-xs font-medium text-center border border-gray-300 bg-white min-w-[140px]" onClick={() => coluna.sortable && handleSort(coluna.id)}><div className="inline-flex items-center gap-1 h-full">{coluna.label} {coluna.sortable && <SortIcon column={coluna.id} />}</div></TableHead>;
                  })}
                </TableRow>
                <TableRow className="bg-white border-b">
                  {colunasOrdenadas.map((coluna) => {
                    if (coluna.id === "selecao") return <TableHead key="filter-selecao" className="bg-white sticky left-0 z-40 w-10 min-w-[40px] max-w-[40px] border border-gray-300 p-1"></TableHead>;
                    if (coluna.id === "acoes") return <TableHead key="filter-acoes" className="bg-white sticky left-[40px] z-40 w-10 min-w-[40px] max-w-[40px] border border-gray-300 p-1"></TableHead>;
                    if (coluna.id === "titulo") return <TableHead key="filter-titulo" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Input value={filtroTitulo} onChange={(e) => setFiltroTitulo(e.target.value)} placeholder="Filtrar tarefa" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "descricao") return <TableHead key="filter-descricao" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Input value={filtroDescricao} onChange={(e) => setFiltroDescricao(e.target.value)} placeholder="Filtrar descrição" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "prioridade") return <TableHead key="filter-prioridade" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-48 p-2"><Select value={filtroPrioridade} onValueChange={setFiltroPrioridade}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem><SelectItem value="Baixa">Baixa</SelectItem><SelectItem value="Média">Média</SelectItem><SelectItem value="Alta">Alta</SelectItem></SelectContent></Select></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "status") return <TableHead key="filter-status" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-52 p-2"><Select value={filtroStatus} onValueChange={setFiltroStatus}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem><SelectItem value="Pendente">Pendente</SelectItem><SelectItem value="Em Andamento">Em Andamento</SelectItem><SelectItem value="Concluída">Concluída</SelectItem><SelectItem value="Cancelada">Cancelada</SelectItem></SelectContent></Select></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "grupo_atividade_nome") return <TableHead key="filter-grupo" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Select value={filtroGrupo} onValueChange={setFiltroGrupo}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{grupos.map((grupo) => <SelectItem key={grupo} value={grupo}>{grupo}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "tipo_tarefa_nome" || coluna.id === "tipo") return <TableHead key={`filter-${coluna.id}`} className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Select value={filtroTipoTarefa} onValueChange={setFiltroTipoTarefa}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{tiposTarefa.map((tipo) => <SelectItem key={tipo} value={tipo}>{tipo}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "setor_nome") return <TableHead key="filter-setor" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Select value={filtroSetor} onValueChange={setFiltroSetor}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{setores.map((setor) => <SelectItem key={setor} value={setor}>{setor}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "area_nome") return <TableHead key="filter-area" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Select value={filtroArea} onValueChange={setFiltroArea}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{areas.map((area) => <SelectItem key={area} value={area}>{area}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "responsavel") return <TableHead key="filter-responsavel" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Input value={filtroResponsavel} onChange={(e) => setFiltroResponsavel(e.target.value)} placeholder="Filtrar responsável" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu></TableHead>;
                    if (coluna.id === "solicitante") return <TableHead key="filter-solicitante" className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="center" className="w-56 p-2"><Input value={filtroSolicitante} onChange={(e) => setFiltroSolicitante(e.target.value)} placeholder="Filtrar solicitante" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu></TableHead>;
                    return <TableHead key={`filter-${coluna.id}`} className="bg-white border border-gray-300 p-1 text-center"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6 text-slate-300"><Filter className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger></DropdownMenu></TableHead>;
                  })}
                </TableRow>
              </TableHeader>
              <TableBody>
                ...
                <TableRow
                  ...
                  onTouchEnd={(event) => handleRowTouch(tarefa, event)}>
                    {colunasOrdenadas.map((coluna) => {
                    if (coluna.id === "selecao") return <TableCell key={`${tarefa.id}-selecao`} className="p-0 bg-white text-muted-foreground font-medium text-center sticky left-0 z-20 w-10 min-w-[40px] max-w-[40px] align-middle px-0 border border-gray-300" onClick={(event) => event.stopPropagation()} onTouchEnd={(event) => event.stopPropagation()}><Checkbox checked={selectedItems.includes(tarefa.id)} onCheckedChange={(checked) => setSelectedItems((prev) => checked ? [...prev, tarefa.id] : prev.filter((id) => id !== tarefa.id))} className="peer shrink-0 shadow disabled:opacity-50 h-4 w-4 rounded-full border-2 border-gray-400 shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 disabled:cursor-not-allowed disabled:opacity-70 data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground" /></TableCell>;
                    if (coluna.id === "acoes") return <TableCell key={`${tarefa.id}-acoes`} className="p-0 bg-white text-muted-foreground font-medium text-center sticky left-[40px] z-20 w-10 min-w-[40px] max-w-[40px] align-middle px-0 border border-gray-300" onClick={(event) => event.stopPropagation()} onTouchEnd={(event) => event.stopPropagation()}><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6"><MoreVertical className="w-3.5 h-3.5 text-slate-600" /></Button></DropdownMenuTrigger><DropdownMenuContent align="start"><DropdownMenuItem asChild className="text-xs"><Link to={createPageUrl(`LancamentoTarefaForm?id=${tarefa.id}`)}>Editar</Link></DropdownMenuItem><DropdownMenuItem onClick={() => onDelete(tarefa.id)} className="text-xs text-red-600">Excluir</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell>;
                    return <TableCell key={`${tarefa.id}-${coluna.id}`} className="p-2 text-gray-700 text-xs align-middle px-2 h-7 border border-gray-300 bg-white min-w-[140px]">{renderCell(tarefa, coluna.id)}</TableCell>;
                  })}
                  </TableRow>
                )}
              </TableBody>
