              ...
              <TableHeader className="sticky top-0 z-30 bg-white shadow-sm">
                <TableRow className="sticky top-0 z-40 bg-white border-b">
                  {colunasOrdenadas.map((coluna) => {
                    ...
                    return <TableHead key={coluna.id} className="sticky top-0 relative h-10 align-middle text-gray-900 px-6 pr-6 text-xs font-medium text-center border border-gray-300 bg-white min-w-[160px] z-30" onClick={() => coluna.sortable && handleSort(coluna.id)}><div className="inline-flex items-center justify-center gap-1 h-full w-full">{coluna.label} {coluna.sortable && <SortIcon column={coluna.id} />}</div>{filterControl ? <div className="absolute right-1 top-1/2 -translate-y-1/2 z-20" onClick={(e) => e.stopPropagation()}>{filterControl}</div> : null}</TableHead>;
                  })}
                </TableRow>
              </TableHeader>
              <TableBody>
