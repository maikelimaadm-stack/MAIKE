              ...
              <TableHeader className="bg-white">
                <TableRow className="sticky top-0 z-30 bg-white">
                  {colunasOrdenadas.map((coluna) => {
                    if (coluna.id === "selecao") return <TableHead key="selecao" className="sticky top-0 left-0 z-40 h-10 p-0 bg-white text-muted-foreground font-medium text-center w-10 min-w-[25px] max-w-[25px] align-middle [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-0 px-0 border-x border-b border-gray-300 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-white after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-gray-300"><Checkbox checked={selectedItems.length === tarefasFiltradas.length && tarefasFiltradas.length > 0} onCheckedChange={toggleSelectAll} className="peer shrink-0 shadow disabled:opacity-50 h-4 w-4 rounded-full border-2 border-gray-400 shadow-lg\nfocus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400\ndisabled:cursor-not-allowed disabled:opacity-70\ndata-[state=checked]:bg-primary\ndata-[state=checked]:text-primary-foreground" /></TableHead>;
                    if (coluna.id === "acoes") return <TableHead key="acoes" className="sticky top-0 left-0 z-40 h-10 p-0 bg-white text-muted-foreground font-medium text-center w-10 min-w-[25px] max-w-[25px] align-middle [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-0 px-0 border-x border-b border-gray-300 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-white after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-gray-300"></TableHead>;
                    const isRight = coluna.align === "right";
                    const filterControl = coluna.id === "titulo" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Input value={filtroTitulo} onChange={(e) => setFiltroTitulo(e.target.value)} placeholder="Filtrar tarefa" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu> : coluna.id === "descricao" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Input value={filtroDescricao} onChange={(e) => setFiltroDescricao(e.target.value)} placeholder="Filtrar descrição" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu> : coluna.id === "prioridade" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48 p-2"><Select value={filtroPrioridade} onValueChange={setFiltroPrioridade}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem><SelectItem value="Baixa">Baixa</SelectItem><SelectItem value="Média">Média</SelectItem><SelectItem value="Alta">Alta</SelectItem></SelectContent></Select></DropdownMenuContent></DropdownMenu> : coluna.id === "status" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-52 p-2"><Select value={filtroStatus} onValueChange={setFiltroStatus}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem><SelectItem value="Pendente">Pendente</SelectItem><SelectItem value="Em Andamento">Em Andamento</SelectItem><SelectItem value="Concluída">Concluída</SelectItem><SelectItem value="Cancelada">Cancelada</SelectItem></SelectContent></Select></DropdownMenuContent></DropdownMenu> : coluna.id === "grupo_atividade_nome" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Select value={filtroGrupo} onValueChange={setFiltroGrupo}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{grupos.map((grupo) => <SelectItem key={grupo} value={grupo}>{grupo}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu> : coluna.id === "tipo_tarefa_nome" || coluna.id === "tipo" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Select value={filtroTipoTarefa} onValueChange={setFiltroTipoTarefa}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{tiposTarefa.map((tipo) => <SelectItem key={tipo} value={tipo}>{tipo}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu> : coluna.id === "setor_nome" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Select value={filtroSetor} onValueChange={setFiltroSetor}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{setores.map((setor) => <SelectItem key={setor} value={setor}>{setor}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu> : coluna.id === "area_nome" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Select value={filtroArea} onValueChange={setFiltroArea}><SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Todos" /></SelectTrigger><SelectContent><SelectItem value="__TODOS__">Todos</SelectItem>{areas.map((area) => <SelectItem key={area} value={area}>{area}</SelectItem>)}</SelectContent></Select></DropdownMenuContent></DropdownMenu> : coluna.id === "responsavel" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Input value={filtroResponsavel} onChange={(e) => setFiltroResponsavel(e.target.value)} placeholder="Filtrar responsável" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu> : coluna.id === "solicitante" ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-300 hover:text-slate-400"><Filter className="w-2 h-2" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56 p-2"><Input value={filtroSolicitante} onChange={(e) => setFiltroSolicitante(e.target.value)} placeholder="Filtrar solicitante" className="h-8 text-xs" /></DropdownMenuContent></DropdownMenu> : null;
                    return <TableHead key={coluna.id} className="sticky top-0 h-10 relative align-middle text-gray-900 px-6 pr-6 text-xs font-medium text-center border-x border-b border-gray-300 bg-white min-w-[160px] z-30 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-white after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-gray-300" onClick={() => coluna.sortable && handleSort(coluna.id)}><div className="inline-flex items-center justify-center gap-1 h-full w-full">{coluna.label} {coluna.sortable && <SortIcon column={coluna.id} />}</div>{filterControl ? <div className="absolute right-1 top-1/2 -translate-y-1/2 z-20 flex items-center gap-0.5" onClick={(e) => e.stopPropagation()}>{filterControl}{(coluna.id === "titulo" && filtroTitulo) || (coluna.id === "descricao" && filtroDescricao) || (coluna.id === "prioridade" && filtroPrioridade !== "__TODOS__") || (coluna.id === "status" && filtroStatus !== "__TODOS__") || (coluna.id === "grupo_atividade_nome" && filtroGrupo !== "__TODOS__") || ((coluna.id === "tipo_tarefa_nome" || coluna.id === "tipo") && filtroTipoTarefa !== "__TODOS__") || (coluna.id === "setor_nome" && filtroSetor !== "__TODOS__") || (coluna.id === "area_nome" && filtroArea !== "__TODOS__") || (coluna.id === "responsavel" && filtroResponsavel) || (coluna.id === "solicitante" && filtroSolicitante) ? <Button variant="ghost" size="icon" className="h-3 w-3 min-w-3 p-0 text-slate-400 hover:text-slate-600" onClick={(e) => {e.stopPropagation();if (coluna.id === "titulo") setFiltroTitulo("");if (coluna.id === "descricao") setFiltroDescricao("");if (coluna.id === "prioridade") setFiltroPrioridade("__TODOS__");if (coluna.id === "status") setFiltroStatus("__TODOS__");if (coluna.id === "grupo_atividade_nome") setFiltroGrupo("__TODOS__");if (coluna.id === "tipo_tarefa_nome" || coluna.id === "tipo") setFiltroTipoTarefa("__TODOS__");if (coluna.id === "setor_nome") setFiltroSetor("__TODOS__");if (coluna.id === "area_nome") setFiltroArea("__TODOS__");if (coluna.id === "responsavel") setFiltroResponsavel("");if (coluna.id === "solicitante") setFiltroSolicitante("");}}><X className="w-2 h-2" /></Button> : null}</div> : null}</TableHead>;
                  })}
                </TableRow>
              </TableHeader>
              <TableBody>
