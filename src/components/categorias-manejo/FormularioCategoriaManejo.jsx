...
export default function FormularioCategoriaManejo({
  ...
  const handleChange = (field, value) => {
    const nextValue =
      UPPERCASE_FIELDS.includes(field) && typeof value === "string" ? value.toUpperCase() : value;
  ...
  const getFieldClassName = (field, baseClass = "") => {
    return `${baseClass} ${invalidFields.includes(field) ? "border-red-500 bg-red-50 focus-visible:ring-red-500" : ""
      }`.trim();
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-1">
            ...
            <div className="space-y-1">
              ...
              <Select value={formData.categoria_oficial || "__VAZIO__"} onValueChange={(value) => handleChange("categoria_oficial", value === "__VAZIO__" ? "" : value)}>
                <SelectTrigger className={getFieldClassName("categoria_oficial", "h-7 text-xs")}>
                  <SelectValue placeholder="SELECIONE" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="__VAZIO__" className="text-xs">SELECIONE</SelectItem>
