...
import { MoreVertical, Search, Shield, User, Lock } from "lucide-react";
...
export default function TabelaUsuarios({ usuarios = [], permissoes = [], currentUser, onEdit, onDelete, isLoading }) {
  ...
  const getPermissaoUsuario = (userEmail) => {
    return permissoes.find(p => p.user_email === userEmail);
  };
  ...
  const contarModulos = (userEmail) => {
    const permissao = getPermissaoUsuario(userEmail);
    if (permissao?.is_admin) return 'Todos';
    return permissao?.modulos_permitidos?.length || 0;
  };
              ...
              <AnimatePresence>
                {isLoading ? (
                  ...
                  filteredUsuarios.map((usuario) => {
                    ...
                    const modulosPermitidos = contarModulos(usuario.email);
