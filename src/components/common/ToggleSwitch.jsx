...
export default function ToggleSwitch({ checked, disabled = false, onChange, className = "" }) {
  return (
    <button
      ...
      className={`rounded-full relative inline-block transition-colors h-4 w-8 ${checked ? "bg-green-500" : "bg-slate-300"} ${disabled ? "opacity-70 cursor-default" : "cursor-pointer"} ${className}`}
