...
const COLUNAS_DISPONIVEIS = [
  ...
  { id: "nome_tipo", label: "Tipo", default: true, sortable: true, align: "left", width: 220 },
  { id: "grupo_atividade_nome", label: "Grupo", default: true, sortable: true, align: "left", width: 220 },
  { id: "ativo", label: "Ativo", default: true, sortable: true, align: "left", width: 100 },
  { id: "descricao", label: "Descrição", default: true, sortable: true, align: "left", width: 260 },
  { id: "created_date", label: "Criado em", default: false, sortable: true, align: "left", width: 170 },
  { id: "updated_date", label: "Atualizado em", default: true, sortable: true, align: "left", width: 170 },
];

const DEFAULT_VISIBLE_COLUMNS = COLUNAS_DISPONIVEIS.filter((c) => c.default).map((c) => c.id);
const COLUMN_WIDTHS_KEY = "colunas_largura_tipos_tarefa";
...
export default function TabelaTiposTarefa({ tipos = [], grupos = [], onEdit, onDelete, showConfigColunas, setShowConfigColunas }) {
  ...
  const getFieldValue = (item, colunaId) => {
    ...
    if (colunaId === "created_date") return formatarDataHora(item.created_date);
    if (colunaId === "updated_date") return formatarDataHora(item.updated_date);
    return "";
  };
  ...
  const columnOptions = useMemo(() => {
    ...
    COLUNAS_DISPONIVEIS.filter((c) => !c.fixo).forEach((col) => {
      opts[col.id] = [...new Set(tipos.map((item) => getFieldValue(item, col.id)).filter(Boolean))].sort((a, b) =>
  ...
  const tiposOrdenados = useMemo(() => {
    ...
    return sorted;
  }, [tiposFiltrados, sortConfig, grupos]);
      ...
      >
        ...
        <PopoverContent align="end" side="bottom" sideOffset={4} className="w-[310px] p-0 z-[9999]">
          ...
          <div className="p-2 space-y-2">
            <input value={buscaFiltroMenu} onChange={(e) => setBuscaFiltroMenu(e.target.value)} placeholder="PESQUISAR" className="flex h-8 w-full rounded-md border border-input bg-transparent px-3 py-1 text-xs uppercase shadow-sm transition-colors" />
    ...
    <div className="space-y-1 overflow-hidden">
      <div className="flex justify-between items-center px-1 gap-2 flex-wrap">
        <div className="text-xs text-slate-500">
          {tiposFiltrados.length} de {tipos.length} registros
        </div>
              ...
              <Table ref={tableRef} className={`w-full ${isMobile ? "min-w-[720px]" : "min-w-[900px]"} border-separate border-spacing-0 table-fixed`}>
                        ...
                        return (
                          <TableHead key="selecao" style={{ width: 25, minWidth: 25, maxWidth: 25 }} className="sticky top-0 z-40 h-7 p-0 bg-white text-muted-foreground font-medium text-center align-middle px-0 border-r border-b border-gray-200">
                            <div className="flex items-center justify-center w-full h-full">
                              <Checkbox checked={selectedItems.length === tiposFiltrados.length && tiposFiltrados.length > 0} onCheckedChange={toggleSelectAll} className="peer shrink-0 shadow disabled:opacity-50 h-4 w-4 rounded-full border-2 border-gray-400 data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground" />
                            </div>
                          </TableHead>
                        );
                      }
                ...
                <TableBody>
                  {tiposOrdenados.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={colunasOrdenadas.length} className="text-center py-8 text-xs text-slate-400 border border-gray-300">Nenhum tipo encontrado</TableCell>
                    </TableRow>
                  ) : (
