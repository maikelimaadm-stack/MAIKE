...
const COLUNAS_DISPONIVEIS = [
  ...
  { id: "nome_grupo", label: "Nome", default: true, sortable: true, align: "left", width: 220 },
  { id: "ativo", label: "Ativo", default: true, sortable: true, align: "left", width: 100 },
  { id: "descricao", label: "Descrição", default: true, sortable: true, align: "left", width: 240 },
  { id: "observacoes", label: "Observações", default: true, sortable: true, align: "left", width: 240 },
...
const COLUMN_WIDTHS_KEY = "colunas_largura_grupos_atividades";
...
export default function TabelaGruposAtividades({ grupos = [], onEdit, onDelete, showConfigColunas, setShowConfigColunas }) {
  ...
  const toggleColuna = (colunaId) => {
    ...
    localStorage.setItem("colunas_visiveis_grupos_atividades", JSON.stringify(novas));
  };
  ...
  const columnOptions = useMemo(() => {
    ...
    COLUNAS_DISPONIVEIS.filter((c) => !c.fixo).forEach((col) => {
      opts[col.id] = [...new Set(grupos.map((item) => getFieldValue(item, col.id)).filter(Boolean))].sort((a, b) =>
                        ...
                        return (
                          <TableHead key="selecao" style={{ width: 25, minWidth: 25, maxWidth: 25 }} className="sticky top-0 z-40 h-7 p-0 bg-white text-muted-foreground font-medium text-center align-middle px-0 border-r border-b border-gray-200">
                            <div className="flex items-center justify-center w-full h-full">
                              <Checkbox checked={selectedItems.length === gruposFiltrados.length && gruposFiltrados.length > 0} onCheckedChange={toggleSelectAll} className="peer shrink-0 shadow disabled:opacity-50 h-4 w-4 rounded-full border-2 border-gray-400 data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground" />
                            </div>
                          </TableHead>
                        );
                      }
