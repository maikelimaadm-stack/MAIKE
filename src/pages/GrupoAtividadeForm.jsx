...
export default function GrupoAtividadeForm() {
  ...
  const handleSave = async () => {
    ...
    if (isEdit) {
      await base44.entities.GrupoAtividade.update(id, form);
    } else {
      await base44.entities.GrupoAtividade.create(form);
    }
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center justify-between bg-white rounded px-3 py-2 border-b">
        <h1 className="text-lg font-bold text-slate-900">{isEdit ? "Editar" : "Novo"} Grupo de Atividades</h1>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => { window.location.href = createPageUrl("GruposAtividades"); }}>Cancelar</Button>
