...
export default function UnidadesMedida() {
  ...
  const [colunasOrdem, setColunasOrdem] = useState(() => { const s = localStorage.getItem(`${STORAGE_PREFIX}_ordem`); if (s) try { return JSON.parse(s); } catch { /* falha ignorada intencionalmente: operação best-effort */ } return COLUNAS_DISPONIVEIS.map(c => c.id); });
  const [colunasVisiveis, setColunasVisiveis] = useState(() => { const s = localStorage.getItem(`${STORAGE_PREFIX}_visiveis`); if (s) try { return Array.from(new Set([...JSON.parse(s), ...DEFAULT_VISIBLE])); } catch { /* falha ignorada intencionalmente: operação best-effort */ } return DEFAULT_VISIBLE; });
