                    ...
                    <Table>
                      ...
                      <TableBody>
                        ...
                        <TableRow className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted font-bold bg-slate-50 border-t-2 border-slate-200">
                          <TableCell className="p-2 [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px] px-2 py-1 text-gray-700 text-xs align-middle border-r border-b border-gray-300 whitespace-normal break-words">TOTAL GERAL</TableCell>
                          <TableCell className="p-2 [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px] px-2 py-1 text-gray-700 text-xs align-middle border-r border-b border-gray-300 whitespace-normal break-words">{Object.values(porApartacao).reduce((s, lotes) => s + Object.keys(lotes).length, 0).toLocaleString('pt-BR')}</TableCell>
                          <TableCell className="p-2 [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px] px-2 py-1 text-gray-700 text-xs align-middle border-r border-b border-gray-300 whitespace-normal break-words">{totalAnimais.toLocaleString('pt-BR')}</TableCell>
                          <TableCell className="text-xs text-center py-2 font-mono text-slate-800">{pesagensFiltradas.length > 0 ? Math.min(...pesagensFiltradas.map((p) => p.peso || 999999).filter((p) => p > 0)).toLocaleString('pt-BR') : 0}</TableCell>
                          <TableCell className="text-xs text-center py-2 font-mono text-slate-800">{pesagensFiltradas.length > 0 ? Math.max(...pesagensFiltradas.map((p) => p.peso || 0)).toLocaleString('pt-BR') : 0}</TableCell>
                          <TableCell className="text-xs text-center py-2 font-mono font-bold text-slate-800">{pesoMedio.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}</TableCell>
                          <TableCell className="text-xs text-center py-2 font-mono font-bold text-slate-800">{arrobaMediaGeral.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
