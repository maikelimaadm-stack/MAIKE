  ...
  const deleteMutation = useMutation({
    mutationFn: async (tarefa) => {
      await base44.entities.HistoricoTarefaMapa.create({
        ...
        descricao: "Tarefa removida pela gestão de tarefas.",
      });
      return base44.entities.TarefaMapa.delete(tarefa.id);
    },
    onSuccess: () => {
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
            <div className="space-y-1 md:col-span-2">
              ...
              <div className="relative">
                ...
                <Input value={search} onChange={(e) => setSearch(e.target.value)} className="h-8 text-xs pl-8" placeholder="Título, grupo, área..." />
              </div>
            </div>
            ...
            <div className="flex items-end">
              <Button variant="outline" size="sm" className="h-8 text-xs w-full" onClick={() => { setSearch(""); setFStatus("todos"); setFPrioridade("todas"); setFGrupo("todos"); }}>
                Limpar Filtros
              </Button>
            </div>
          </div>
      ...
      <Card>
        <CardContent className="p-0 overflow-auto">
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow><TableCell colSpan={9} className="text-xs py-1 border border-gray-300 text-center">Carregando...</TableCell></TableRow>
              ) : filtered.length === 0 ? (
                <TableRow><TableCell colSpan={9} className="text-xs py-1 border border-gray-300 text-center">Nenhuma tarefa encontrada</TableCell></TableRow>
              ) : filtered.map((tarefa) => {
                const prioridade = normalizeTaskPriority(tarefa.prioridade);
                return (
                  <TableRow key={tarefa.id} className="hover:bg-gray-50">
                    <TableCell className="text-xs py-1 border border-gray-300 font-medium">{tarefa.titulo}</TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300"><Badge className={`text-[10px] ${STATUS_CORES[tarefa.status] || STATUS_CORES.Pendente}`}>{tarefa.status}</Badge></TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300">
                      <div className="flex items-center gap-2">
                        {getIconePrioridade(prioridade)?.icone_url && <img src={getIconePrioridade(prioridade).icone_url} alt={prioridade} className="w-4 h-4 object-contain" />}
                        <Badge className={`text-[10px] ${PRIORIDADE_CORES[prioridade]}`}>{prioridade}</Badge>
                      </div>
                    </TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300">{tarefa.grupo_atividade_nome || "-"}</TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300">{tarefa.tipo_tarefa_nome || tarefa.tipo || "-"}</TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300">{tarefa.area_nome || "-"}</TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300">{tarefa.responsavel || "-"}</TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300">{tarefa.data_prevista || "-"}</TableCell>
                    <TableCell className="text-xs py-1 border border-gray-300 text-center">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem className="text-xs text-red-600" onClick={() => window.confirm("Excluir tarefa?") && deleteMutation.mutate(tarefa)}>
                            Excluir
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
