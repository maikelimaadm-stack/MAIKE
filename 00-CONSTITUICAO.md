...
Transformar o MAKGESTAO em um sistema completo e independente, com o mesmo padrão
arquitetural validado no PROJETOMG, removendo integralmente a dependência da Base44.
...
### P2 — O molde já existe
O PROJETOMG resolveu esta migração. Não se inventa arquitetura nova; replica-se a
que já funciona. Divergência do molde exige justificativa escrita.
...
1. Esta constituição
2. `07-DO-NOT-DO.md` — proibições explícitas
3. `08-REGRAS-DE-IA.md` — restrições adicionais para agentes
4. `DECISIONS.md` — decisões registradas
5. `ROADMAP-SAAS.md` — plano de execução

Em conflito, prevalece o documento de menor número.

## 4. Emenda

Alterar esta constituição exige: registro em `DECISIONS.md`, justificativa escrita
