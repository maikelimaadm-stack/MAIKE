...
**Atualizado em:** 2026-09-10 (**P0, P1, P2, P3, P4.0 e P4.1 mergeadas**, incluindo as correções **P4.1-R1** e **P4.1-R2** — o navegador autentica contra o backend próprio e `Setor` já é capacidade nativa · **P4 em execução**; **DEPLOY-MIGRATION-01 fechado** — a barreira de migrations aplicou sozinha a migration pendente em produção, e a ONDA 1 está liberada)
...
| Missão em execução | **P4 — Mapa Core Native Persistence** — fatias **P4.0** (com P4.0-R1 e P4.0-R2) e **P4.1** (com P4.1-R1 e P4.1-R2) concluídas e mergeadas. **DEPLOY-MIGRATION-01** fechado: 01A na PR #19 (`ae897d0`) e 01B na PR #20 (`257eaaa`), com o pre-deploy provado em produção. **P4.2** (AreaPastagem) liberada |
...
| **DEPLOY-MIGRATION-01** | **Fechado.** 01A (runner, gate, provas — D-PROD-28) na PR #19; 01B (ativação no Railway — D-PROD-29) nas PRs #20, #21 e #22. O deployment `f0dab2d0`, originado do merge `257eaaa`, aplicou `20260909201703_p4_1_r2_setor_tipo_default` pelo pre-deploy, em container separado do `backend:start`. Produção passou de **2** para **3** migrations e `Setor.tipo` ganhou o default `'Próprio'` sem ninguém rodar SQL à mão. Fica a regra do achado: configuração de deploy só vale no próximo deployment **originado da fonte** — `redeploy` reexecuta a especificação antiga e não prova ativação | fechado |
