...
**Atualizado em:** 2026-09-10 (**P0, P1, P2, P3, P4.0 e P4.1 mergeadas**, incluindo as correções **P4.1-R1** e **P4.1-R2** — o navegador autentica contra o backend próprio e `Setor` já é capacidade nativa · **P4 em execução**; o blocker antes da próxima onda é **DEPLOY-MIGRATION-01B**, a ativação da barreira de migrations no Railway)
...
| Missão em execução | **P4 — Mapa Core Native Persistence** — fatias **P4.0** (com P4.0-R1 e P4.0-R2) e **P4.1** (com P4.1-R1 e P4.1-R2) concluídas e mergeadas. **DEPLOY-MIGRATION-01A** (barreira de migrations) mergeada na PR #19 (`ae897d0`); **DEPLOY-MIGRATION-01B** (ativação no Railway) configurada e aguardando a evidência do primeiro deployment originado da fonte |
...
| Próxima fatia autorizável | P4.3 — PontoReferencia + LinhaGeografica + ConfiguracaoIcone — **não iniciada** |
...
| DBT-29 | **Parcialmente fechado na P4.2.** `AreaPastagem` saiu: renomear um setor agora reescreve o `setor_nome` das áreas na mesma transação, nativamente (D-PROD-30). Os outros cinco destinos seguem. `syncEntityReferences` continua na Base44 e continua sendo chamada quando o nome de um setor muda: os **seis** destinos denormalizados (`AreaPastagem`, `PontoSuplementacao`, `Lote`, `LancamentoTarefa`, `MovimentacaoMapa`, `MovimentacaoPecuaria`) ainda são entidades de lá. É a única razão pela qual `Setor` permanece em `allowedBase44Entities`. **Não são os mesmos quatro da guarda de exclusão** — ver D-PROD-25 §N | P4.2 |
| **DEPLOY-MIGRATION-01** | **01A fechada; 01B em fechamento.** O runner, o gate e as provas estão na `main` (D-PROD-28). No Railway já estão gravados `MIGRATION_DATABASE_URL = ${{ DATABASE_URL }}` e o Pre-deploy Command `npm run deploy:migrate` (D-PROD-29). O que ainda **não** está registrado é um deployment mostrando o pre-deploy executar e aplicar `20260909201703_p4_1_r2_setor_tipo_default`. Um `redeploy` **não** serve de prova: ele reexecuta a especificação do deployment antigo, e isso foi estabelecido por controle negativo — com fixture inválida na 6543 o deployment terminou verde, ou seja, a barreira não rodou. Até as linhas de log existirem, produção segue com **2** migrations contra **3** na `main` | antes da ONDA 1 |
...
| DBT-31 | `AreaPastagem.quantidade_atual` e `status_ocupacao` continuam editáveis pelo cliente. Os dois descrevem ocupação, que é consequência de `Lote` — entidade da P5. Enquanto o lote não for nativo, o backend não tem como derivá-los sem consultar a Base44 (proibido); inventá-los produziria número errado com cara de calculado (D-PROD-30) | P5 |
| DBT-32 | `syncEntityReferences` continua na Base44 e continua citando `AreaPastagem` entre os destinos — ramificação que, desde a P4.2, escreve em dado que ninguém lê. Inofensiva; sai quando os outros cinco destinos migrarem. A propagação que importa passou a ser nativa e transacional (`propagarNomeDoSetor`) | P5–P6 |
